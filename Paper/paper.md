...
# Software Design
Developed explicitly with compactness and self-sufficiency in mind, with no other dependencies than the standard libraries of .NET 10, the `Socnet.se` client builds on standard object-oriented principles to allow for future extensions. The codebase is organized in two projects: `Socnet.Core`, a class library containing the data structures, ideal blocks and search algorithms, and `Socnet.CLIconsole`, the command-line client that parses and executes commands. The repository also contains `Socnet.Tests`, a suite of automated unit tests.

Since direct blockmodeling is computationally demanding, the search algorithms are designed for efficiency. During a search, the client keeps running tallies of the ties between each actor and each cluster. As a result, moving an actor from one cluster to another only requires re-evaluating the blocks in the rows and columns of the two affected clusters, and most ideal blocks can be evaluated in constant time from these tallies. Independent search runs, such as the random starts of the local optimization heuristics, are executed in parallel on all available processor cores. Each run gets its own seed, derived from the random seed, so results remain reproducible. The number of cores used can be limited with the `threads` argument of `bminit()` and `coreperi()`.

Developed explicitly with compactness and self-sufficiency in mind, with no other dependencies than the standard libraries of .NET 8.0, the `Socnet.se` client builds on standard object-oriented principles to allow for future extensions. The codebase is organized in two projects: `Socnet.Core`, a class library containing the data structures, ideal blocks and search algorithms, and `Socnet.CLIconsole`, the command-line client that parses and executes commands. (Additionally, the software repository also contains `Socnet.Tests`, a suite of automated unit tests.) Specifically, the codebase has been prepared with three types of extensions in mind: implementing new ideal blocks, new goodness-of-fit measures, and new search heuristics. More detailed technical instructions for adding new ideal blocks are also provided in the `CONTRIBUTING.md` file that is available in the `Socnet.se` repository.

Several design choices shape `Socnet.se`. First, it is a standalone command-line client rather than a package for a specific language: analyses can be run interactively, as script files, or from R and Python, and all data are exchanged as plain text files that other network analysis tools can read and write. Second, ideal blocks are treated as interchangeable components defined independently of the goodness-of-fit measure, so that the same blockimage can be fitted using either Hamming distances or weighted correlations, and new blocks – such as those developed for extended core-periphery models – can be added without changing the search algorithms. Third, version 2.0 was designed to reproduce the results of version 1.4 exactly for all ideal blocks and goodness-of-fit measures, while replacing its internals for performance: blocks are evaluated incrementally, and independent searches run in parallel, with per-task seeds that keep results reproducible and at below-normal process priority, so that long searches do not lock up the user's computer.

## Ideal blocks
Each ideal block is implemented as a separate class that inherits from the abstract class `IdealBlock`, which specifies the properties and methods shared by all ideal blocks. To add a new ideal block, a new class is created in the `Socnet.Core/Blocks/` folder that implements the block-specific properties and goodness-of-fit method(s). To make a new ideal block available when constructing blockimages, the ideal block is registered in the `BlockFactory` class, which makes it available when constructing blockimages.

## Goodness-of-fit measures
`Socnet.se` currently includes two measures, Hamming distances and weighted correlations, but additional measures can be implemented as well. A new measure is added to the `GofMethod` enumeration and then implemented in all ideal block classes that should support it. The evaluation of complete blockmodels in `SearchState` (used during searches) and `BlockmodelEvaluator` (used for final and tested blockmodels) is then extended with the new measure. Finally, it is added as an option to the `bminit()` command in the `BlockmodelingSession` class.

## Search heuristics
New heuristics and algorithms for finding optimal partitions can also be implemented in `Socnet.se`. Each search approach is implemented as a separate class in the `Socnet.Core/Blockmodeling/` folder, like the existing `ExhaustiveSearch` and `LocalSearch` classes. It operates on a `SearchState` object, which handles the incremental evaluation of candidate partitions. A new search approach is added to the `SearchType` enumeration and dispatched from the `BlockmodelSearch` class, which also runs independent searches in parallel. Finally, it is exposed as an option in the `bminit()` command.
...
# AI Usage Disclosure
Versions 1.x of `Socnet.se`, including the command language, the implementation of all ideal blocks and goodness-of-fit measures, and the search heuristics, were developed by the main author without any use of generative AI tools. The methods themselves are based on the main author's previous methodological work [@Nordlund2018; @Nordlund2020; @Estevez2025]. The current version (2.0) is a complete rewrite of version 1.4 using Claude Code (Anthropic). The rewrite was specified, directed and reviewed by the main author, who set its requirements (identical command syntax, functionality and output as version 1.4, improved performance and search algorithms, and new features such as multi-core searches) and tested the software on empirical analyses. The automated tests in the `Socnet.Tests` project were generated in full by Claude Code; they validate version 2.0 against the code of version 1.4, confirming identical goodness-of-fit values and ideal patterns for all ideal blocks. The main author takes full responsibility for the software. AI assistance (Claude) was also used for drafting the revised Software Design section and for proofreading and improving readability of the manuscript text.
