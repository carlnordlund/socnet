...
The codebase has been prepared with three types of extensions in mind: new ideal blocks, new goodness-of-fit measures, and new search heuristics. A short introduction to each follows below; more detailed technical instructions for adding new ideal blocks are provided in the `CONTRIBUTING.md` file in the `Socnet.se` repository.
...
# AI Usage Disclosure
Versions 1.x of `Socnet.se` were developed without any use of generative AI tools. This includes the command language, the implementation of all ideal blocks and goodness-of-fit measures, and the search heuristics. The implemented methods are based on the authors' previous methodological work [@Nordlund2018; @Nordlund2020; @Estevez2025]. The current version (2.0) is a reimplementation of version 1.4. It keeps the exact command syntax, functionality and output of that version, which served both as its specification and as the reference for validating it. The code for version 2.0 was generated in full by Claude Code (Anthropic), under the direction and review of the main author. The main author defined the requirements of the rewrite: identical functionality and results as version 1.4, improved performance and search algorithms, and new features such as multi-core searches. The main author also tested the software on empirical analyses. The automated tests in the `Socnet.Tests` project were also generated by Claude Code. They validate version 2.0 against the code of version 1.4, confirming identical goodness-of-fit values and ideal patterns for all ideal blocks. The main author takes full responsibility for the software. AI assistance (Claude) was also used for drafting the revised Software Design section and for proofreading and improving readability of the manuscript text.
